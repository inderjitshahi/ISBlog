import React from 'react';
import Image from 'next/image';
import { AiOutlineSearch } from 'react-icons/ai';
import { MdMarkEmailUnread } from 'react-icons/md';
import logo from '../public/images/18233896_v1032-v286-aew-071-health-logo.jpg';
import image from '../public/images/10782951_19199360.jpg';
function Recommendations(props) {
    return (
        <div className='p-1 fixed top-0'>
            <button className='button my-5 w-full items-center'>Get Unlimited Access</button>
            <div className='flex items-center gap-5 border rounded-full px-2 py-1 w-full'>
                <AiOutlineSearch />
                <input placeholder='search' className='outline-none' type="text" />
            </div>
            <div className='flex flex-col items-center space-y-1 mt-5'>
                <div className='relative w-20 h-20 overflow-hidden rounded-full mb-1'>
                    <Image
                        src={logo}
                        fill
                        alt="author image"
                    />
                </div>
                <span className='font-semibold'>Inderjit Shahi</span>
                <span className='text-gray-500'>1M followers</span>
                <div className='flex space-x-2'>
                    <button className='button bg-green-500 hover:bg-green-400'>follow</button>
                    <button className='button bg-green-500 hover:bg-green-400 rounded-full'><MdMarkEmailUnread /></button>
                </div>
            </div>
            <div className='mt-10'>
                <p className='mb-2'>More From ISBlog</p>
                <div className='border-b-2 rounded-xl p-1 cursor-pointer active:scale-95 transition transform duration-100 ease-out flex'>
                    <div className=''>
                        <div className='flex space-x-2 items-center text-sm'>
                            <div className='relative h-5 w-5 rounded-full overflow-hidden'>
                                <Image
                                    src={logo}
                                    fill
                                    alt='article logo'
                                />
                            </div>
                            <span>Inderjit Shahi</span>
                        </div>
                        <p className='font-semibold'>Ultimate Course For Js</p>
                    </div>
                    <div className='relative h-16 w-16 rounded-full overflow-hidden'>
                        <Image
                            src={image}
                            fill
                            alt='article logo'
                        />
                    </div>
                </div>
            </div>
        </div>
    );
}

export default Recommendations;